...
public class Main {
	...
	public static void main(String[] args) {
		...
		PromotionAbsolute miPromo1 = new PromotionAbsolute(Preferency.COMBATE,500);
		
		miPromo1.loadSight("Namekusei", miListaDeAtracciones);
		miPromo1.loadSight("Universo6", miListaDeAtracciones);
		miPromo1.loadSight("Inframundo", miListaDeAtracciones);
		
		ArrayList<OfferdableItem> miListaDeOfferdables = new ArrayList<OfferdableItem>();
		
		miListaDeOfferdables.add(miPromo1);
		miListaDeOfferdables.addAll(miListaDeAtracciones);
		
		
		
		
		Collections.sort(miListaDeOfferdables,new ComparatorPrefCostTime(goku.getPref()));
		Collections.sort(miListaDeAtracciones,new ComparatorPrefCostTime(goku.getPref()));
		
		for(OfferdableItem s: miListaDeOfferdables) {
			System.out.println(s);
			System.out.println("----------------------------------------------------------------------------");
			if(s.canBeBoughtBy(goku)) {
				s.boughtBy(goku);
			}
		}
		
		System.out.println(goku.getStrMySights() +" Dinero gastado: "+goku.getSpentMoney() +" Tiempo invertido: "+goku.getSpentTime());
